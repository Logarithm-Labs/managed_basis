...
contract ProdTest is Test {
    ...
    string constant rpcUrl = "https://arb-mainnet.g.alchemy.com/v2/0THPn5xrU18mpUj1mXpN5pLp_45qrsvz";
    ...
    function test_replay_utilize() public {
        vm.createSelectFork(rpcUrl, 312674029);
        ...
        (uint256 sizeInTokens, uint256 netBalance, uint256 markPrice, IHedgeManager.AdjustPositionPayload memory params)
        = abi.decode(
            hex"",
            (uint256, uint256, uint256, IHedgeManager.AdjustPositionPayload)
        );
        console.log("sizeInTokens", sizeInTokens);
        console.log("netBalance", netBalance);
        console.log("markPrice", markPrice);
        console.log("sizeDeltaInTokens", params.sizeDeltaInTokens);
        console.log("collateralDeltaAmount", params.collateralDeltaAmount);
        console.log("isIncrease", params.isIncrease);

        LogarithmOracle oracle = LogarithmOracle(Arb.ORACLE);
        uint256 usdcValue = oracle.convertTokenAmount(
            ArbAddresses.WBTC,
            ArbAddresses.USDC,
            OffChainPositionManager(Arb.HEDGE_MANAGER_HL_USDC_WBTC).decreaseSizeMin()
        );
        console.log("usdcValue", usdcValue);

        // vm.startPrank(0x195fa26fb43D15567C3a99049f57B8Db992643d6);
        // OffChainPositionManager(Arb.HEDGE_MANAGER_HL_USDC_WBTC).reportStateAndExecuteRequest(
        //     sizeInTokens, netBalance, markPrice, params
        // );

        console.log(OffChainPositionManager(Arb.HEDGE_MANAGER_HL_USDC_WBTC).decreaseSizeMin());
        console.log(OffChainPositionManager(Arb.HEDGE_MANAGER_HL_USDC_WBTC).decreaseCollateralMin());
        console.log(OffChainPositionManager(Arb.HEDGE_MANAGER_HL_USDC_WBTC).limitDecreaseCollateral());
    }
    ...
    function test_getState() public {
        ...
        // OffChainConfig config = OffChainConfig(Arb.CONFIG_HL);
        // console.log(config.decreaseSizeMin());
        // console.log(config.decreaseCollateralMin());
