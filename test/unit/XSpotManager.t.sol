...
contract XSpotManagerTest is ForkTest {
    ...
    function test_buy_request(uint256 amount) public {
        amount = bound(amount, 0, TEN_THOUSAND_USDC);
    ...
    function test_buy_response(uint256 amount) public {
        ...
        spotManager.receiveMessage(swapper, abi.encode(amount));
        uint256 productLD = productSD * 1e12; // 18 decimals
        assertEq(strategy.buyAssetDelta(), amount);
        assertEq(strategy.buyProductDelta(), productLD);
    }
    ...
    function test_sell_response(uint256 amount) public {
        ...
        bytes memory composeMsg = abi.encode(productSD);
        ...
        assertEq(strategy.sellAssetDelta(), assetLD);
        assertEq(strategy.sellProductDelta(), amount);
    }
}
